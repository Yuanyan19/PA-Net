...
from Code.model_pa.backbone.models.ResNest import resnest50
...
class PA_Net(nn.Module):
    def __init__(self, channel=32, n_class=1):
        ...
        # ---- Partial Decoder ----       
        self.eca = eca_layer(channel, k_size=5)
    ...
    def forward(self, x):
        ...
        # ---- low-level features ----
        x = self.resnest.maxpool(x)      # bs, 64, 88, 88
        x1 = self.resnest.layer1(x)      # bs, 256, 88, 88

        # ---- high-level features ----
        x2 = self.resnest.layer2(x1)     # bs, 512, 44, 44
        x3 = self.resnest.layer3(x2)     # bs, 1024, 22, 22
        x4 = self.resnest.layer4(x3)     # bs, 2048, 11, 11
